...
ALLTHEALLELES <- read.structure(file = "../MungedData/1randSS.str", n.ind = 338, n.loc = 21, onerowperind = T, col.lab = 1, col.pop = 2, row.marknames = 1, NA.char = "-9", col.others = 3)

groupcov <- read.csv(x = groupcov, file = "../Figures/GroupCoV.csv")
expcov <- read.csv(x = ExpCoV, file = "../Figures/ExpCoV.csv")
popcov <- read.csv(x = popcov, file = "../Figures/PopCoV.csv")
...
ggplot(popHS_Cov, aes(ExpHet, COV, col=locals)) + geom_point() + geom_smooth(method = 'lm', se = F)

write.csv(popHS_Cov, "../Figures/PopHS_Cov.csv")
groupHS_Cov <- full_join(hsgroup, groupcov)

ggplot(groupHS_Cov, aes(ExpHet, COV, col=locals)) + geom_point() + geom_smooth(method = 'lm', se = F)
