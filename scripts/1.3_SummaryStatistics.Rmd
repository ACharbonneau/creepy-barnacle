...
poppr_filter_ATA <- as.genclone(FilterATA)
...
inputPanel(
  selectInput("Grouplevel", "Grouping:", choices = c("Pop"="~Pop", "Group"="~Group" ), selected = "~Group"),
...
renderText( { capture.output(
  poppr.amova(poppr_filter_ATA, as.formula(input$Grouplevel), filter = T, missing = input$miss, cutoff= as.numeric(input$cutoff)), type=c("message"))})


renderPlot({ AMOVA_ATA <- poppr.amova(poppr_filter_ATA, as.formula(input$Grouplevel), filter = T, missing = input$miss, cutoff= as.numeric(input$cutoff))
 
  amova.test <- randtest( AMOVA_ATA ) # Test for significance
  plot(amova.test) })

renderPrint( {AMOVA_ATA <- poppr.amova(poppr_filter_ATA, as.formula(input$Grouplevel), filter = T, missing = input$miss, cutoff= as.numeric(input$cutoff))
  randtest( AMOVA_ATA )})

renderPrint( {poppr.amova(poppr_filter_ATA, as.formula(input$Grouplevel), filter = T, missing = input$miss, cutoff= as.numeric(input$cutoff)) })
```
