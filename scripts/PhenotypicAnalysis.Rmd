...
# landra effect
Nat_pop_chains[ , 12 ] <- Nat_pop_chains[ , 12 ] + Nat_glmm$Sol[, 3]

# maritimus effect
Nat_pop_chains[ , c( 2, 14 ) ] <- Nat_pop_chains[ , c( 2, 14 ) ] + Nat_glmm$Sol[, 4]

# rostratus effect
Nat_pop_chains[ , 5 ] <- Nat_pop_chains[ , 5 ] + Nat_glmm$Sol[, 5]
...
JeffSatPlot + theme_light() + theme( axis.text.x = element_text( angle = -50, size = 14, hjust = -.05 ),
                                 axis.text.y = element_text( size = 14 ),
                                 axis.title.y = element_text( size = 14 ),
                                 strip.text.x = element_text( angle = 0, size = 10 ) )
...
PS_figure + theme_light() + scale_colour_tableau()
