...
RawData <- filter( FlowData, !is.na( GTF ) & 
                             !is.na( msDOYGS )  ) %>% 
           select( ID, GTF, Geography, Seedstock, GrowthEnvironment, msDOYGS, 
                   msDaysVernNoNA, Pop, Experiment, Habitat, PTF, msDOYPS, PTG, 
...
```{r PlotNatPopMeans, echo=FALSE, fig.path="../Figures/", fig.keep="last", fig.width=11, eval=F, message=F}
plot.new()
...
```{r TableofVern, echo=FALSE, warning=FALSE, message=FALSE}
...
natcov <- group_by(native.data, Pop, locals) %>% summarise(avg=mean(GTF), stdd=sd(GTF), min=min(GTF), max=max(GTF), COV=(stdd/avg * 100))

lanmar.data <- filter(RawData, locals=="lanmar") %>% arrange(Order)

lanmarcov <- group_by(lanmar.data, Pop, locals) %>% summarise(avg=mean(GTF), stdd=sd(GTF), min=min(GTF), max=max(GTF), COV=(stdd/avg * 100))

weed.data <- filter(RawData, locals=="raphNN") %>% arrange(Order)

weedcov <- group_by(weed.data, Pop, locals) %>% summarise(avg=mean(GTF), stdd=sd(GTF), min=min(GTF), max=max(GTF), COV=(stdd/avg * 100))

crop.data <- filter(RawData, Species=="sativus") %>% arrange(Order)

cropcov <- group_by(crop.data, Pop, locals) %>% summarise(avg=mean(GTF), stdd=sd(GTF), min=min(GTF), max=max(GTF), COV=(stdd/avg * 100))

ExpCoV <- group_by(RawData, Pop, Experiment, locals) %>% summarise(avg=mean(GTF), stdd=sd(GTF), min=min(GTF), max=max(GTF), COV=(stdd/avg * 100))

groupcov <- group_by(RawData, locals) %>% summarise(avg=mean(GTF), stdd=sd(GTF), min=min(GTF), max=max(GTF), COV=(stdd/avg * 100))

popcov <- group_by(RawData, Pop, locals) %>% summarise(avg=mean(GTF), stdd=sd(GTF), min=min(GTF), max=max(GTF), COV=(stdd/avg * 100))
...
boxplot(lanmar.data$GTF ~ lanmar.data$Order, frame=F, axes=F, ylim=c(0,350), 
        varwidth=TRUE, at=c(1, 1.8, 2.7))

axis(side = 2, line=-1, tck=.95, col='gray85')

boxplot(lanmar.data$GTF ~ lanmar.data$Order, add=T, frame=F, axes=F, ylim=c(0,350), 
        varwidth=TRUE, at=c(1, 1.8, 2.7),
...
boxplot(native.data$GTF ~ native.data$Order, add=T, frame=F, axes=F, ylim=c(0,200),
...
boxplot(weed.data$GTF ~ weed.data$Order, frame=F, axes=F, ylim=c(0,200))
axis(side = 2, line=-1, tck=.9, col="gray85", yaxp=c(0, 150, 3))

boxplot(weed.data$GTF ~ weed.data$Order, add=T, frame=F, axes=F, ylim=c(0,200),
        col=hmcols[c(
...
boxplot(crop.data$GTF ~ crop.data$Order, frame=F, axes=F, ylim=c(0,200),
        varwidth=TRUE, boxwex=.7, at=c(seq(from=.9, to=18.9, by=.9)))
axis(side = 2, line=-1, tck=.95, col='gray85')
par(new=TRUE, cex=.7)

boxplot(crop.data$GTF ~ crop.data$Order, add=T, frame=F, axes=F, ylim=c(0,200),
...
```{r ExperimentalVar, echo=FALSE, fig.path="../Figures/", fig.keep="all", message=FALSE, warning=FALSE, fig.height=10, fig.width=14}

group_by(RawData, Pop) %>%
ggplot( aes(Pop:Experiment, GTF, col=Experiment)) +
  geom_boxplot() +
...
group_by(RawData, Pop) %>%
ggplot( aes(Experiment, GTF, col=Experiment)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  facet_wrap("Pop")
...
```{r SmallExperimentalVar, echo=FALSE, fig.path="../Figures/", fig.keep="all", message=FALSE, warning=FALSE, fig.height=10, fig.width=14}

filter(RawData, Pop=="BINY"| Pop=="MAES"| Pop=="SAES") %>%
group_by( Pop) %>%
ggplot( aes(Experiment, GTF, col=Experiment)) +
