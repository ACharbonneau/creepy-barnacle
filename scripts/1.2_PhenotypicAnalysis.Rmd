...
natcov <- group_by(native.data, Pop) %>% summarise(avg=mean(GTF), stdd=sd(GTF), min=min(GTF), max=max(GTF), COV=(stdd/avg * 100))

lanmar.data <- filter(RawData, locals=="lanmar") %>% arrange(Order)

lanmarcov <- group_by(lanmar.data, Pop) %>% summarise(avg=mean(GTF), stdd=sd(GTF), min=min(GTF), max=max(GTF), COV=(stdd/avg * 100))

weed.data <- filter(RawData, locals=="raphNN") %>% arrange(Order)

weedcov <- group_by(weed.data, Pop) %>% summarise(avg=mean(GTF), stdd=sd(GTF), min=min(GTF), max=max(GTF), COV=(stdd/avg * 100))

crop.data <- filter(RawData, Species=="sativus") %>% arrange(Order)

cropcov <- group_by(crop.data, Pop) %>% summarise(avg=mean(GTF), stdd=sd(GTF), min=min(GTF), max=max(GTF), COV=(stdd/avg * 100))

CoV <- group_by(RawData, Pop, Experiment, locals) %>% summarise(avg=mean(GTF), stdd=sd(GTF), min=min(GTF), max=max(GTF), COV=(stdd/avg * 100))

popcov <- rbind(natcov, lanmarcov, weedcov, cropcov)

write.csv(x = CoV, file = "../figures/CoV.csv")
