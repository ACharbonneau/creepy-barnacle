...
group_by(native.data, Pop) %>% summarise(avg=mean(GTF), stdd=sd(GTF), min=min(GTF), max=max(GTF))

lanmar.data <- filter(RawData, locals=="lanmar") %>% arrange(Order)

group_by(lanmar.data, Pop) %>% summarise(avg=mean(GTF), stdd=sd(GTF), min=min(GTF), max=max(GTF))

weed.data <- filter(RawData, locals=="raphNN") %>% arrange(Order)

group_by(weed.data, Pop) %>% summarise(avg=mean(GTF), stdd=sd(GTF), min=min(GTF), max=max(GTF))

crop.data <- filter(RawData, Species=="sativus") %>% arrange(Order)

group_by(crop.data, Pop) %>% summarise(avg=mean(GTF), stdd=sd(GTF), min=min(GTF), max=max(GTF))
```
