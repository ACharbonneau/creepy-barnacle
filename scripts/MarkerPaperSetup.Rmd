...
JustSNPs <- Genotypes[,c(1:19)]
JustSSR <- Genotypes[c(1, 20:35)]

write.csv(JustSSR, "../MungedData/JustSSR.csv", row.names = F, quote=F)
system("python biallele.py ../MungedData/JustSSR.csv ../MungedData/BiSSR.csv")

JustSSRFix <- read.csv("../MungedData/BiSSR.csv", header = T)
colnames(JustSSRFix) <- c("SSR.ID", colnames(JustSSRFix[2:113]))

BialleleGene <- dplyr::full_join( JustSNPs, JustSSRFix )

SubsetBialleleGene <- droplevels( dplyr::filter( BialleleGene, 
                                          LocData != "UKspecies" &
                                          Pop != "NZIL"))
...
write.table(t(SubsetBialleleGene[, c(7:ncol(SubsetBialleleGene))]), "../MungedData/Marker.geno", row.names = F, col.names = F, sep="", na = "9")

write.table(cbind(SubsetBialleleGene$SSR.ID, rep("U", length(SubsetBialleleGene$Pop)), SubsetBialleleGene$Pop), "../MungedData/Marker.ind", row.names = F, quote = F, col.names = F, sep = "\t")

write.table(cbind( colnames(SubsetBialleleGene)[7:131], 
